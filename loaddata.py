...
path="E:\\python\\kaggle\\Kaggle-101-master\\Titanic\\" 
...
def Mother(df, keep_scaled=False):
    df['Mother'] = None
    for i in range(0,len(df.index)) :
        if (df['Title'][i] == "Mrs" )& (df['Parch'][i] > 0) :
            df['Mother'][i] = 1
        else :
            df['Mother'][i] = 0    
    df['Mother'] = df['Mother'].astype(float)        
   # df=pd.concat([df,pd.get_dummies(df['Mother']).rename(columns=lambda x:'Mother_'+str(x))],axis=1)
    return df   
